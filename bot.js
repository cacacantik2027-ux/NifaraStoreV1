...
bot.onText(/^\/settings(@\S+)?$/, async (msg) => {
  if (!isAdmin(msg.chat.id) || !isAllowedAdmin(msg.from.id)) return;
  clearAdminEdit();
...
async function handleSettingsCallback(data, chatId, msgId) {
  ...
  if (data === 'settings_edit_webappUrl') {
    await promptAdminInput('webappUrl',
  ...
  if (data === 'settings_prod_add') {
    ...
    await bot.sendMessage(chatId, '1/4 — Kirim *nama produk* baru:\n\n_Ketik /batal untuk membatalkan._', { parse_mode: 'Markdown' });
    return;
  }
...
async function handleAdminEditInput(msg) {
  ...
  if (field === 'webappUrl') {
    if (!text) { await bot.sendMessage(ADMIN_ID, '⚠️ Kirim teks URL, atau ketik "-" untuk menghapus (atau /batal).'); return true; }
    if (text === '-') {
      SETTINGS.webappUrl = '';
      saveSettings();
      await backToMain(`✅ Link Web App dihapus. Tombol "🌐 Buka Web Store" tidak akan muncul lagi di menu.`);
      return true;
    }
    if (!isValidWebAppUrl(text)) {
      await bot.sendMessage(ADMIN_ID, '⚠️ URL harus diawali `https://` (Telegram menolak URL http:// biasa untuk Web App). Kirim ulang atau /batal.', { parse_mode: 'Markdown' });
      return true;
    }
    SETTINGS.webappUrl = text;
    saveSettings();
    await backToMain(`✅ Link Web App disimpan. Tombol "🌐 Buka Web Store" sekarang muncul di menu utama.`);
    return true;
  }
