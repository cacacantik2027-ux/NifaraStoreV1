...
const DEFAULT_SITE_TEXTS = {
  ...
  alurStep7Title   : 'Isi form pengiriman',
  alurStep7Desc    : 'Nama, alamat, kurir, hingga catatan tambahan.',
  alurStep8Title   : 'Pesanan diproses',
  alurStep8Desc    : 'Dikirim via JNE / J&T / SiCepat / GoSend.',
...
async function renderSettingsView(chatId, msgId, view, prefixText) {
  ...
  try {
    await bot.editMessageText(finalText, { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: keyboard });
  } catch (_) {
    await bot.sendMessage(chatId, finalText, { parse_mode: 'Markdown', reply_markup: keyboard });
  }
}
...
async function updateAdminPrompt(promptText) {
  ...
  try {
    await bot.editMessageText(text, { chat_id: adminEdit.chatId, message_id: adminEdit.backMsgId, parse_mode: 'Markdown', reply_markup: kb });
  } catch (_) {
    const sent = await bot.sendMessage(adminEdit.chatId, text, { parse_mode: 'Markdown', reply_markup: kb });
    adminEdit.backMsgId = sent.message_id;
  }
}
...
async function handleSettingsCallback(data, chatId, msgId) {
  ...
  if (data.startsWith('settings_prod_edit_')) {
    ...
    const fields = ['name', 'price', 'desc', 'stock', 'photo'];
    ...
    const labelMap = { name: 'nama', price: 'harga', desc: 'deskripsi', stock: 'info stok' };
    if (field === 'photo') {
...
async function handleAdminEditInput(msg) {
  ...
  const prodFieldMap = { prod_name: 'name', prod_price: 'price', prod_desc: 'desc', prod_stock: 'stock' };
  ...
  if (field === 'prod_add') {
    if (!text) { await warn('⚠️ Kirim teks (atau /batal).'); return true; }
    const steps  = ['name', 'price', 'desc', 'stock'];
    const labels = {
      name  : '*nama produk*',
    ...
    const newProduct = { id: makeId('p'), name: d.name, price: d.price, desc: d.desc, stock: d.stock, photo: '', variants: [] };
...
bot.on('message', async (msg) => {
  ...
  // ────────────────────────────────────────────────────────────────
  if (adminEdit && chatId === adminEdit.chatId && isAllowedAdmin(msg.from.id)) {
