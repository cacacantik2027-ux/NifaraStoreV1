...
## Website etalase (public/) — v6

Halaman web sekarang **otomatis mengambil data dari `settings.json`** lewat
endpoint `GET /api/site` (bukan lagi teks statis di HTML), dan dipecah jadi
beberapa halaman:

- `index.html` — Beranda (hero + mockup chat)
- `produk.html` — Daftar produk (tanpa harga)
- `cara-order.html` — Nota alur pemesanan
- `pembayaran.html` — Metode bayar, termasuk 2 kartu QRIS dengan foto QR

**Semua teks di halaman web** (nav, judul, deskripsi, tombol, langkah cara
order, footer, dll) bisa diedit dari bot lewat **/settings → 📝 Edit Teks
Website**, dikelompokkan per bagian. Foto QR untuk 2 metode QRIS diisi
lewat **/settings → 💳 Metode Pembayaran → (pilih QRIS) → 🖼️ Edit Foto/QR**
(upload foto langsung di chat Telegram).

Foto yang diupload admin (foto toko, foto produk, foto QR) disimpan sebagai
`file_id` Telegram dan ditampilkan di web lewat proxy `GET /photo/:fileId`
— jadi `BOT_TOKEN` tidak pernah terekspos ke browser.
